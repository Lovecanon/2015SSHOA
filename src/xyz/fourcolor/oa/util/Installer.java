...
public class Installer {
	...
	public void install() {
		...
		session.save(new Privilege("初始化密码", "/user_edit", menu3));
		...
		menu2 = new Privilege("申请模版管理", "/template_list", menu);
		menu3 = new Privilege("起草申请", "/flow_templateList", menu);
		menu4 = new Privilege("待我审批", "/flow_myTaskList", menu);
		menu5 = new Privilege("我的审批查询", "/flow_myApplicationList", menu);
		session.save(menu);
		session.save(menu1);
		session.save(menu2);
		session.save(menu3);
	...
	public static void main(String[] args) {
		/*
		 * 不需要tomcat启动就可以运行初始化数据的功能，命令行输入java命令就ok
		 * 所以可以自己new一个容器对象
		 */
